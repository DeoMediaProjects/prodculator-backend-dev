...
def upgrade() -> None:
    ...
    if "data_sources" not in inspector.get_table_names():
        ...
        # Table already exists, create a reference to it for bulk_insert
        data_sources = sa.table(
            ...
            sa.Column("sync_schedule", sa.String()),
            sa.Column("last_tested_at", sa.DateTime()),
            sa.Column("last_test_result", sa.String()),
    ...
    conn = op.get_bind()
    result = conn.execute(sa.text("SELECT COUNT(*) FROM data_sources"))
    count = result.scalar()
