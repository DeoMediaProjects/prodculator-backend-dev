...
Return a JSON object with ONLY these keys:
{
  ...
  "executiveSummary_keyInsights": "2-4 short paragraphs for financiers and producers. The FIRST paragraph MUST be labelled **Story Summary** and give a concise, accurate summary of the script's story from beginning to end: the setting, the central character or characters (names only from script_characters), the core conflict, and how it develops and resolves. It should read as if the whole script was read closely, not skimmed. AFTER the story summary, add the production and recommendation paragraphs, each prefixed with its own short bold label using double asterisks (for example **Primary Recommendation**, **Second Territory**, **Production Complexity**). Full sentences only. Do NOT use em-dashes or en-dashes (the long dash characters) anywhere; use commas, colons, parentheses, or separate sentences, whatever is grammatically natural. No bullet points, no numbered lists, no hyphens at line starts. Separate each paragraph with a blank line. Write as a senior production consultant would in a formal executive briefing.",
